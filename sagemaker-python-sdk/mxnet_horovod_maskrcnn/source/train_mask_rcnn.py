...
def get_dataset(args):
    ...
    val_dataset = gdata.COCOInstance(
    ...
    val_metric = COCOInstanceMetric(
        val_dataset, args.save_prefix + "_eval", use_ext=False, starting_id=starting_id
    )
    if args.horovod and MPI:
...
def validate(net, val_data, async_eval_processes, ctx, eval_metric, logger, epoch, best_map, args):
    ...
    for ib, batch in enumerate(val_data):
        ...
        # update metric
        for det_bbox, det_id, det_score, det_mask, det_info in zip(
            det_bboxes, det_ids, det_scores, det_masks, det_infos
        ):
    ...
    if args.horovod and MPI is not None:
        ...
        if hvd.rank() == 0:
            logger.info(
                "[Epoch {}] Validation Inference cost: {:.3f}".format(epoch, (time.time() - tic))
            )
