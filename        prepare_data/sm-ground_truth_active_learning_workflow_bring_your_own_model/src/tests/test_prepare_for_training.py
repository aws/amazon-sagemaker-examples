...
def test_prepare_for_training(monkeypatch):
    def mock_copy(*args, **kwargs):
        ...
        s3r.Object("output", "active-learning-0ypM8t7c/training_input.manifest").put(
            Body=augmented_input.getvalue().encode()
        )
        print(
            "Copy with transform mocked out source {} dest {} query {}".format(source, dest, query)
        )
        return mock_input
    ...
    event = {
        ...
        "meta_data": {
            ...
            "training_config": {"TrainingJobName": "job-name", "S3OutputPath": "s3://output/"},
        },
    }
    ...
    training_input = (
        s3r.Object("output", "active-learning-0ypM8t7c/training_input.manifest")
        .get()["Body"]
        .read()
    )
