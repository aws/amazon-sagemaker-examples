...
def publish_game_server_status(status, server_type):
    print "in publish_game_server_status with hostname=" + public_hostname + " port=" + str(
        public_port
    ) + " region=" + region + " status=" + status + " type=" + server_type
    data = []
    data.append(
        {
    ...
    try:
        ...
        response = queue.send_message(MessageBody=str(data), MessageGroupId="gsGrp_" + region)
