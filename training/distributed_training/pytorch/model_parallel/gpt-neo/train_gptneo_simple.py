...
from smdistributed.modelparallel.torch.nn.huggingface.gptneo import (
    translate_hf_gptneo_state_dict_to_smdistributed,
...
def save(
    ...
    if partial:
        ...
        if smp.rank() == 0:
            save_dict["model"] = (
                translate_state_dict_to_hf_gptneo(model_state_dict, seq_length)
                if translate_to_hf
...
def load_model_and_optimizer(
    ...
    if load_model:
        checkpointed_model = (
            translate_hf_gptneo_state_dict_to_smdistributed(checkpoint["model"], seq_length)
...
def main():
    ...
    model_config = GPTNeoConfig(
        ...
        attention_types=[[["global", "local"], int(args.num_layers / 2)]],
        num_heads=args.num_heads,
        intermediate_size=8192,
        activation_function="gelu_new",
        resid_pdrop=args.resid_pdrop,
        embed_dropout=args.embd_pdrop,
    ...
    if args.ci:
        print(f"[SMP_METRIC]__GPT-Neo__Time_to_train__{time_to_train}")
        print(f"[SMP_METRIC]__GPT-Neo__samples/second__{throughput}")
        print(f"[SMP_METRIC]__GPT-Neo__Loss__{loss}")
