...
def get_data_key_plaintext(master_key: str, ciphertext: bytes) -> bytes:
    ...
    plaintext = kms.decrypt(KeyId=master_key, CiphertextBlob=ciphertext)["Plaintext"]
    return plaintext
...
def main():
    ...
    C = get_data_key_ciphertext(bucket=hyp["key_bucket"], key=hyp["ciphertext_s3_key"])

    # get plaintext data key
    P = get_data_key_plaintext(master_key=hyp["master_key_id"], ciphertext=C)

    # read encrypted binary data
    data_file = os.path.join(data_dir, hyp["train_channel"], hyp["train_file"])
