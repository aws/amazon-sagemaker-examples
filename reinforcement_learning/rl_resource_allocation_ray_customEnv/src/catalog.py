...
class ModelCatalog:
    ...
    def get_action_dist(
        ...
        **kwargs
        ...
        # Box space -> DiagGaussian OR Deterministic.
            ...
            if action_space.dtype.name.startswith("int"):
                ...
                if dist_type is None:
                    dist_cls = TorchDiagGaussian if framework == "torch" else DiagGaussian
        ...
        elif dist_type in (MultiActionDistribution, TorchMultiActionDistribution) or isinstance(
            action_space, (Tuple, Dict)
        ):
            return ModelCatalog._get_multi_action_distribution(
                (MultiActionDistribution if framework == "tf" else TorchMultiActionDistribution),
                action_space,
                config,
                framework,
            )
        # Simplex -> Dirichlet.
            ...
            if framework == "torch":
                # TODO(sven): implement
                raise NotImplementedError("Simplex action spaces not supported for torch.")
            dist_cls = Dirichlet
    ...
    def get_action_shape(action_space: gym.Space, framework: str = "tf") -> (np.dtype, List[int]):
        ...
        if isinstance(action_space, Discrete):
            ...
            size = int(size)
            return dl_lib.int64 if all_discrete else dl_lib.float32, (None, size)
        else:
        ...
        if model_config.get("custom_model"):
            ...
            if not issubclass(model_cls, ModelV2):
                if framework not in ["tf", "tf2", "tfe"] or not issubclass(
                    ...
                    raise ValueError(
                        "`model_cls` must be a ModelV2 sub-class, but is" " {}!".format(model_cls)
                    )
            ...
            if framework in ["tf2", "tf", "tfe"]:
                ...
                if model_config.get("use_lstm") or model_config.get("use_attention"):
                    ...
                    from ray.rllib.models.tf.recurrent_net import LSTMWrapper, Keras_LSTMWrapper
                    ...
                    # wrap with ModelV2 LSTM/GTrXL (attention) wrappers.
                        ...
                        model_cls = ModelCatalog._wrap_if_needed(
                            wrapped_cls,
                            LSTMWrapper if model_config.get("use_lstm") else AttentionWrapper,
                        )
                ...
                raise NotImplementedError(
                    "`framework` must be 'tf2|tf|tfe|torch', but is " "{}!".format(framework)
                )
    ...
    def get_preprocessor_for_space(
        ...
        for k in options.keys():
            if k not in MODEL_DEFAULTS:
                raise Exception(
                    "Unknown config key `{}`, all keys: {}".format(k, list(MODEL_DEFAULTS))
                )
        ...
        logger.debug(
            "Created preprocessor {}: {} -> {}".format(prep, observation_space, prep.shape)
        )
        return prep

    @staticmethod
    @PublicAPI
    def register_custom_preprocessor(preprocessor_name: str, preprocessor_class: type) -> None:
        ...
        """
        _global_registry.register(RLLIB_PREPROCESSOR, preprocessor_name, preprocessor_class)
    ...
    def _get_v2_model_class(
        ...
        if isinstance(input_space, Tuple) or (
            isinstance(space_to_check, Tuple)
            and any(isinstance(s, Box) and len(s.shape) >= 2 for s in space_to_check.spaces)
        ):
            return ComplexNet

        # Single, flattenable/one-hot-able space -> Simple FCNet.
        if (
            isinstance(input_space, (Discrete, MultiDiscrete))
            or len(input_space.shape) == 1
            or (len(input_space.shape) == 2 and (num_framestacks == "auto" or num_framestacks <= 1))
        ):
    ...
    def _validate_config(config: ModelConfigDict, framework: str) -> None:
        ...
        if config.get("use_attention") and config.get("use_lstm"):
            raise ValueError("Only one of `use_lstm` or `use_attention` may " "be set to True!")
        if framework == "jax":
            if config.get("use_attention"):
                raise ValueError("`use_attention` not available for " "framework=jax so far!")
