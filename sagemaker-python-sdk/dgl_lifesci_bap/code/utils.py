    ...
    if args["dataset"] == "PDBBind":
        ...
        if args["split"] == "sequence":
            train_set, val_set, test_set = [
                Subset(dataset, indices) for indices in dataset.agg_sequence_split
            ]
        elif args["split"] == "structure":
            train_set, val_set, test_set = [
                Subset(dataset, indices) for indices in dataset.agg_structure_split
            ]
            ...
            raise ValueError(
                ...
                'to be "random", "scaffold", "stratified" or "temporal", got {}'.format(
                    args["split"]
                )
            )
...
def load_model(args):
    ...
    if args["model"] == "PotentialNet":
        model = PotentialNet(
            n_etypes=(len(args["distance_bins"]) + 5),
            f_in=args["f_in"],
            f_bond=args["f_bond"],
