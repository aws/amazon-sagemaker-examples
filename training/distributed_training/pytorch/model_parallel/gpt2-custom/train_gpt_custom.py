...
from smdistributed.modelparallel.torch.nn.huggingface.gpt2 import get_hf_gpt2_transformer_layer_hooks
...
from smdistributed.modelparallel.torch.nn.huggingface.gpt2 import (
    ...
    translate_hf_block_state_dict_to_smdistributed,
    translate_layer_state_dict_to_hf_gpt2_block
)
...
def save(
    ...
    if partial:
        ...
        if smp.rank() == 0:
            save_dict["model"] = (
                translate_layer_state_dict_to_hf_gpt2_block(model_state_dict, seq_length)
                if translate_to_hf
...
def load_model_and_optimizer(
    ...
    if load_model:
        checkpointed_model = (
            translate_hf_block_state_dict_to_smdistributed(checkpoint["model"], seq_length)
...
def main():
    ...
    model = AutoModelForCausalLM.from_config(model_config)
    smp.set_tensor_parallelism(model.transformer.h, True)
    smp.tp_register_with_module(GPT2Block, DistributedTransformerLayer, *get_hf_gpt2_transformer_layer_hooks())
