...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Variable to change path to tensors",
    )
    parser.add_argument("--batch_size", type=int, default=64, help="Variable to change batch size")
    parser.add_argument(
        "--num_epochs", type=int, default=10, help="Variable to change number of epochs"
    )
...
def train(batch_size, num_epochs, lr, wd, out_dir):
    ...
    train_dataloader = gluon.data.DataLoader(
        train_dataset_t, batch_size=batch_size, last_batch="rollover", shuffle=True
    )
    test_dataloader = gluon.data.DataLoader(
        test_dataset_t, batch_size=100, last_batch="discard", shuffle=False
    )
    ...
    for epoch in range(num_epochs):
        ...
        for image_batch, label in test_dataloader:
            ...
            loss = loss_function(output, image_batch)
        print(
            "epoch [{}/{}], loss:{:.4f}".format(epoch + 1, num_epochs, mx.nd.mean(loss).asscalar())
        )
