...
def upload_channel(channel_name, x, y):
    ...
    return sagemaker_session.upload_data(
        path=file_path, key_prefix="data/DEMO-keras-cifar10/%s" % channel_name
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ecr-repository",
        help="ECR repo where images will be pushed",
        default="add-ecr-repo-here",
        required=True,
    )
    parser.add_argument("--tf-version", default="latest")
    parser.add_argument(
        "--instance-type", default="local", choices=["local", "ml.c5.xlarge", "ml.p2.xlarge"]
    )
