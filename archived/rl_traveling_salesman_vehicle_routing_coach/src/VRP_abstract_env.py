...
ACTION:
- Go to restaurant j location
- Go to order i location
- Accept order i
...
class VRPEasyEnv(gym.Env):
    ...
    def __init__(
        ...
        self.action_space = spaces.Discrete(n_restaurants + 2 * n_orders)
    ...
    def step(self, action):
        ...
        if action < self.n_restaurants:  # go to restaurant
            b = [self.res_x[action], self.res_y[action]]
            choose_direction = vrp_action_go_from_a_to_b(a, b)
        elif action < (self.n_orders + self.n_restaurants):  # go to order
            order_index = action - self.n_restaurants
            if self.o_status[order_index] == 0:
                # inactive order, don't do anything
                choose_direction = 0
            else:
                b = [self.o_x[order_index], self.o_y[order_index]]
                choose_direction = vrp_action_go_from_a_to_b(a, b)
        elif action < (2 * self.n_orders + self.n_restaurants):  # accept order
            order_index = action - self.n_restaurants - self.n_orders
            choose_direction = 5 + order_index
        else:  # invalid action
            choose_direction = 0

        self.__update_driver_parameters(choose_direction)
