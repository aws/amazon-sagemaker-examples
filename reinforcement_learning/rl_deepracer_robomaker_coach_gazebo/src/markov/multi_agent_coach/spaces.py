...
class ScalableBoxActionSpace(BoxActionSpace):
    def __init__(self, shape: Union[int, np.ndarray], low: Union[None, int, float, np.ndarray] = -np.inf,
                 ...
                 default_action: np.ndarray = None, scale_action_space: bool = False,
                 scaled_up_action_space_bounds: Dict = {ModelMetadataKeys.LOW.value: -np.inf,
    ...
    def scale_action_values(self, actions):
        ...
        for action, low, high, scaled_low, scaled_high in zip(actions, self.low, self.high,
                                                              self.scaled_up_action_space_bounds[
                                                                ModelMetadataKeys.LOW.value],
                                                              self.scaled_up_action_space_bounds[
                                                                ModelMetadataKeys.HIGH.value]):
            scaled_actions.append(self._scale_value(
                action,
                low,
                high,
                scaled_low,
    ...
    def _scale_value(self, action, min_old, max_old, min_new, max_new):
        ...
        """
        if max_old == min_old:
            log_and_exit("Unsupported minimum and maximum action space bounds for scaling values. \
