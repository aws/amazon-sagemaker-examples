...
class LargeOfficeEnv(gym.Env):
    ...
    def __init__(self, config):
       ...
       # Total number of steps
        ...
        #                                         ))
        self.observation_space = spaces.Box(np.array([0, 0, 0, -50]), #chiller outlet temp, boiler outlet, zone 2 mid, outdoor temp
                                            np.array([30, 150, 60, 70]), dtype=np.float32)
        ...
        self.action_space = spaces.Box(np.array([self.chlr_min,self.light_min]), 
                                  np.array([self.chlr_max, self.light_max]), dtype=np.float32)
    ...
    def step(self, action):
        ...
        # state can be all the inputs required to make a control decision
        next_state = np.array([self.outputs[3], self.outputs[5], self.outputs[17], self.outputs[25]])
        # fake state space
