...
class ClippedPPOActionSpaceConfig(ActionSpaceConfigInterface):
    def __init__(self, action_space_type):
        ...
        """
        if action_space_type not in [ActionSpaceTypes.DISCRETE.value, ActionSpaceTypes.CONTINUOUS.value]:
            log_and_exit("Unsupported action space type passed while defining ClippedPPOActionSpaceConfig. \
                action_space_type: {}".format(action_space_type),
                         SIMAPP_SIMULATION_WORKER_EXCEPTION,
    ...
    def get_action_space(self, json_actions):
        ...
        try:
            if self.action_space_type == ActionSpaceTypes.CONTINUOUS.value:
                ...
                if json_actions[ModelMetadataKeys.STEERING_ANGLE.value][ModelMetadataKeys.LOW.value] \
                    >= json_actions[ModelMetadataKeys.STEERING_ANGLE.value][ModelMetadataKeys.HIGH.value] or \
                    ...
                    log_and_exit("Action space bounds are incorrect while defining ClippedPPOActionSpaceConfig. \
                        json_actions: {}".format(json_actions),
                                 SIMAPP_SIMULATION_WORKER_EXCEPTION,
                                 SIMAPP_EVENT_ERROR_CODE_500)
                original_low = np.array([json_actions[ModelMetadataKeys.STEERING_ANGLE.value]
                                                     [ModelMetadataKeys.LOW.value],
                                         json_actions[ModelMetadataKeys.SPEED.value]
                                                     [ModelMetadataKeys.LOW.value]])
                original_high = np.array([json_actions[ModelMetadataKeys.STEERING_ANGLE.value]
                                                      [ModelMetadataKeys.HIGH.value],
                                         json_actions[ModelMetadataKeys.SPEED.value]
                                                     [ModelMetadataKeys.HIGH.value]])

                action_space = ScalableBoxActionSpace(2,
                                                      low,
                                                      high,
                                                      default_action=0.5 * (high + low),
                                                      scale_action_space=True,
                                                      scaled_up_action_space_bounds={ModelMetadataKeys.LOW.value:
                                                                                     original_low,
                                                                                     ModelMetadataKeys.HIGH.value:
                                                                                     original_high})
            ...
            log_and_exit('Error while getting action space in ClippedPPOActionSpaceConfig: {}'.format(ex),
                         SIMAPP_SIMULATION_WORKER_EXCEPTION,
                         SIMAPP_EVENT_ERROR_CODE_500)
...
class SACActionSpaceConfig(ActionSpaceConfigInterface):
    def __init__(self, action_space_type):
        ...
        if action_space_type not in [ActionSpaceTypes.CONTINUOUS.value]:
            log_and_exit("Unsupported action space type passed while defining SACActionSpaceConfig. \
                action_space_type: {}".format(action_space_type),
                         SIMAPP_SIMULATION_WORKER_EXCEPTION,
                         SIMAPP_EVENT_ERROR_CODE_500)
        ...
        try:
            ...
            if json_actions[ModelMetadataKeys.STEERING_ANGLE.value][ModelMetadataKeys.LOW.value] \
                >= json_actions[ModelMetadataKeys.STEERING_ANGLE.value][ModelMetadataKeys.HIGH.value] or \
                    json_actions[ModelMetadataKeys.SPEED.value][ModelMetadataKeys.LOW.value] \
                    >= json_actions[ModelMetadataKeys.SPEED.value][ModelMetadataKeys.HIGH.value]:
                log_and_exit("Action space bounds are incorrect while defining SACActionSpaceConfig. \
                    json_actions: {}".format(json_actions),
                             SIMAPP_SIMULATION_WORKER_EXCEPTION,
                             SIMAPP_EVENT_ERROR_CODE_500)
            low = np.array([json_actions[ModelMetadataKeys.STEERING_ANGLE.value]
                                        [ModelMetadataKeys.LOW.value],
                            json_actions[ModelMetadataKeys.SPEED.value]
                                        [ModelMetadataKeys.LOW.value]])
            high = np.array([json_actions[ModelMetadataKeys.STEERING_ANGLE.value]
                                         [ModelMetadataKeys.HIGH.value],
                            json_actions[ModelMetadataKeys.SPEED.value]
                                        [ModelMetadataKeys.HIGH.value]])
            ...
            log_and_exit('Error while getting action space in SACActionSpaceConfig: {}'.format(ex),
                         SIMAPP_SIMULATION_WORKER_EXCEPTION,
                         SIMAPP_EVENT_ERROR_CODE_500)
