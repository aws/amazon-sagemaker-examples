    ...
    def invoke_at_step(self, step):
        ...
        if self.base_trial._global_to_mode[step][0] == modes.EVAL:
            ...
            predictions = np.argmax(
                self.base_trial.tensor(self.predictions).value(step, mode=modes.EVAL), axis=1
            )
            ...
            if self.previous_accuracy - current_accuracy > self.threshold:
                self.logger.info(
                    f"Step {step}: accuracy dropped by more than {self.threshold}. Current accuracy: {current_accuracy} Previous accuracy {self.previous_accuracy}"
                )
                return True
