...
def get_supported_instance_types(file_name, model_name):
    """
    Returns all supported instance types for a given JumpStart model from the file
    """
    ...
    for m in metadata:
        if m["id"] == model_name:
            ...
            for params in m["parameters"]:
                if params["name"] == "machineTypeInfer":
                    print("\n".join(params["options"]))
                    return params["options"]
    print(
        "Model not found in JumpStart. Check the file again to make sure model exists."
    )
    ...
    if endpoint_name in existing_endpoints:
        ...
        if instance_type.split(".")[1].startswith(("m", "c", "r")):
            if cpu_model:
                ...
                print(
                    f"No CPU model specified for a CPU instance of type {instance_type}"
                )
            ...
            if gpu_model:
                ...
                print(
                    f"No GPU model specified for a GPU instance of type {instance_type}"
                )
        else:
