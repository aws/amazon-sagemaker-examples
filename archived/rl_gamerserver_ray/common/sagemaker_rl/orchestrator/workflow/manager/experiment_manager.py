...
try:
    assert version.parse(sagemaker.__version__) >= version.parse(
        "1.33.0"
    ), "sagemaker version should be >= 1.33.0"
        ...
        if evaluation_state is not None and evaluation_state.endswith("ING"):
            ...
            if evaluation_job_record is not None:
                ...
                if evaluation_state == EvaluationState.EVALUATED:
                    self.experiment_manager.experiment_record._last_evaluation_job_id = (
...
class ExperimentManager:
    ...
    def __init__(
        ...
        self.resource_manager = ResourceManager(
            self.config.get("resource", {}), boto_session=self.boto_session
        )
    ...
    def last_hosted_model_id(self):
        ...
        if (
            self.experiment_record._hosting_state is not None
    ...
    def last_joined_job_id(self):
        ...
        if (
            self.experiment_record._joining_state is not None
    ...
    def last_joined_job_train_data(self):
        record = self.join_db_client.get_join_job_record(
            self.experiment_id, self.last_joined_job_id
        )
    ...
    def _check_if_model_ready(self, model_id):
        ...
        if not model_exist:
            logger.error(
                f"Model with mode_id '{model_id}' was not found in model table. "
    ...
    def predictor(self):
        if self.experiment_record._hosting_endpoint:
            return Predictor(
                endpoint_name=self.experiment_id, sagemaker_session=self.sagemaker_session
            )
        else:
            logger.warning(
    ...
    def ingest_rewards(self, rewards_buffer):
        ...
        timstamp = str(int(time.time()))
        rewards_s3_file_key = (
            f"{self.experiment_id}/rewards_data/{self.experiment_id}-{timstamp}/rewards-{timstamp}"
        )
        body = b""
        ...
        try:
            self.s3_client.put_object(
                Body=body, Bucket=rewards_bucket_name, Key=rewards_s3_file_key
            )
        except ClientError as e:
            error_code = e.response["Error"]["Code"]
            message = e.response["Error"]["Message"]
            raise RuntimeError(
                "Failed to upload rewards data with error {}: {}".format(error_code, message)
            )
