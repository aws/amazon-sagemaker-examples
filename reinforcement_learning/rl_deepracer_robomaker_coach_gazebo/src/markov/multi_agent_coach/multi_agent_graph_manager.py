...
class MultiAgentGraphManager(object):
    ...
    def create_graph(
        ...
        if isinstance(task_parameters, DistributedTaskParameters):
            screen.log_title(
                "Creating graph - name: {} task id: {} type: {}".format(
                    self.__class__.__name__, task_parameters.task_index, task_parameters.job_type
                )
            )
        else:
        ...
        if isinstance(task_parameters, DistributedTaskParameters):
            (
                task_parameters.worker_target,
                task_parameters.device,
    ...
    def _create_graph(
        ...
        env = short_dynamic_import(self.env_params.path)(
            **self.env_params.__dict__, visualization_parameters=self.visualization_parameters
        )
        ...
        for agent_params in self.agents_params:
            ...
            ) and self.memory_backend_params.run_type == str(RunType.ROLLOUT_WORKER):
        ...
        level_manager = MultiAgentLevelManager(
            agents=agents, environment=env, name="main_level", done_condition=self.done_condition
        )
    ...
    def _create_worker_or_parameters_server_tf(task_parameters: DistributedTaskParameters):
        ...
        config.gpu_options.allow_growth = (
            True  # allow the gpu memory allocated for the worker to grow if needed
        )
        ...
        from rl_coach.architectures.tensorflow_components.distributed_tf_utils import (
            ...
            create_cluster_spec,
            create_worker_server_and_device,
        )
        ...
        if task_parameters.job_type == "ps":
            create_and_start_parameters_server(cluster_spec=cluster_spec, config=config)
    ...
    def _create_session_tf(self, task_parameters: TaskParameters):
        ...
        if isinstance(task_parameters, DistributedTaskParameters):
            ...
            if (
                hasattr(self.task_parameters, "checkpoint_restore_path")
            ...
            # regular session
            self.sess = {
                agent_params.name: tf.Session(config=config) for agent_params in self.agents_params
            }
            # set the session for all the modules
            self.set_session(self.sess)
        ...
        if (
            hasattr(self.task_parameters, "checkpoint_save_dir")
    ...
    def create_session(self, task_parameters: TaskParameters):
        ...
        self.checkpoint_saver = {
            agent_params.name: SaverCollection() for agent_params in self.agents_params
        }
        self.checkpoint_state_updater = {
            agent_params.name: None for agent_params in self.agents_params
        }
    ...
    def improve(self):
        ...
        if self.task_parameters.task_index is not None:
            screen.log_title(
                "Starting to improve {} task index {}".format(
        ...
        if self.task_parameters.checkpoint_restore_path:
            ...
            for agent_params in self.agents_params:
                ...
                agent_checkpoint_restore_path = os.path.join(
                    self.task_parameters.checkpoint_restore_path, agent_params.name
                )
                if os.path.isdir(agent_checkpoint_restore_path):
                    # a checkpoint dir
                    if (
                        self.task_parameters.framework_type == Frameworks.tensorflow
                        and "checkpoint" in os.listdir(agent_checkpoint_restore_path)
                    ):
                        ...
                        checkpoint = get_checkpoint_state(
                            agent_checkpoint_restore_path, all_checkpoints=True
                        )

                    if checkpoint is None:
                        raise ValueError(
                            "No checkpoint to restore in: {}".format(agent_checkpoint_restore_path)
                        )
                    ...
                    chkpt_state_reader = CheckpointStateReader(
                        agent_checkpoint_restore_path, checkpoint_state_optional=False
                    )
                ...
                try:
                    ...
                    raise ValueError(
                        "Failed to restore {}'s checkpoint: {}".format(agent_params.name, ex)
                    )
    ...
    def occasionally_save_checkpoint(self):
        ...
        if (
            self.task_parameters.checkpoint_save_secs
            and time.time() - self.last_checkpoint_saving_time
            >= self.task_parameters.checkpoint_save_secs
            and (
                self.task_parameters.task_index == 0  # distributed
    ...
    def save_checkpoint(self):
        ...
        for agent_params in self.agents_params:
            agent_checkpoint_save_dir = os.path.join(
                self.task_parameters.checkpoint_save_dir, agent_params.name
            )
            ...
            if self.checkpoint_state_updater[agent_params.name] is None:
                self.checkpoint_state_updater[agent_params.name] = CheckpointStateUpdater(
                    agent_checkpoint_save_dir
                )
            ...
            if self.num_checkpoints_to_keep < len(
                self.checkpoint_state_updater[agent_params.name].all_checkpoints
            ):
                checkpoint_to_delete = self.checkpoint_state_updater[
                    agent_params.name
                ].all_checkpoints[-self.num_checkpoints_to_keep - 1]
                agent_checkpoint_to_delete = os.path.join(
        ...
        for agent_params in self.agents_params:
            self.checkpoint_state_updater[agent_params.name].update(
    ...
    def emulate_act_on_trainer(
        self, steps: PlayingStepsType, transitions: Dict[str, Transition]
    ) -> None:
        """
        ...
        if self.task_parameters.checkpoint_save_dir and os.path.exists(
            ...
            for agent_params in self.agents_params:
                ...
                if not os.path.exists(agent_checkpoint_save_dir):
                    os.mkdir(agent_checkpoint_save_dir)
                open(
                    os.path.join(agent_checkpoint_save_dir, SyncFiles.TRAINER_READY.value), "w"
                ).close()
        if hasattr(self, "data_store_params"):
            data_store = self.get_data_store(self.data_store_params)
