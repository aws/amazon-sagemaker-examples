...
import os
# disable autotune
os.environ['MXNET_CUDNN_AUTOTUNE_DEFAULT'] = '0' ###Use autotone then allow mxnet to find params and later update.
...
def parse_args():
    ...
    parser.add_argument('--datasetloc', type=str, default='',
                        help='where the dataset is located')
    parser.add_argument('--sm-save', type=str, default='',
                        help='where to save models')
    parser.add_argument('--sm-output', type=str, default='',
                        help='where to save data for sm')
    ...
    parser.add_argument('--horovod', type=str, default='false',
    ...
    args.sm_save = os.path.join(os.environ['SM_MODEL_DIR'],args.sm_save) if 'SM_MODEL_DIR' in keys else args.sm_save
    args.num_workers =  args.num_workers
    args.datasetloc = os.environ['SM_CHANNEL_DATA'] if 'SM_CHANNEL_DATA' in keys else args.datasetloc
    args.gpus = int(os.environ['SM_NUM_GPUS']) if 'SM_NUM_GPUS' in keys else int(args.gpus)
    args.sm_output = os.path.join(os.environ['SM_OUTPUT_DATA_DIR'],args.sm_output) if 'SM_OUTPUT_DATA_DIR' in keys else args.sm_output
...
def get_dataset(args):
    train_dataset = gdata.COCODetection(root=args.datasetloc,splits='instances_train2017', use_crowd=False,skip_empty=True)
    val_dataset = gdata.COCODetection(root=args.datasetloc,splits='instances_val2017', skip_empty=False)
...
def save_params(net, logger, best_map, current_map, epoch, save_interval, prefix,args):
    ...
    if save_interval and (epoch + 1) % save_interval == 0:
        ...
        net.save_parameters(os.path.join(args.sm_save,'{:s}_{:04d}_{:.4f}.params'.format(args.save_prefix, epoch, current_map)))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i, batch in enumerate(train_data):
            if epoch == 0 and i <= lr_warmup:
                ...
                new_lr = base_lr * get_lr_at_iter(i / lr_warmup, args.lr_warmup_factor/args.num_gpus)
        ...
        if (not args.horovod) or hvd.rank() == 0:
            ...
            save_params(net, logger, best_map, current_map, epoch, args.save_interval,
                        os.path.join(args.sm_save,args.save_prefix),args)
...
if __name__ == '__main__':
    ...
    if args.horovod:
        net = get_model(net_name, root=str(hvd.rank()),pretrained_base=True,
                        per_device_batch_size=args.batch_size, **kwargs)
    else:
