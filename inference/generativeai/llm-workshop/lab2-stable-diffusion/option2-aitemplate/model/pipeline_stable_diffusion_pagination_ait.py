...
class StableDiffusionPaginationAITPipeline(StableDiffusionPipeline):
    r"""
    ...
    def __init__(
        ...
        workdir = os.environ.get("AITEMPLATE_WORK_DIR", "/tmp")
    ...
    def __call__(
        ...
        starting_step: Optional[int] = 0, #added step offset
        num_interim_images: Optional[int] = None,
        **kwargs,
    ):
        r"""
        ...
        timesteps = self.scheduler.timesteps[starting_step:]
        ...
        interim_latents = []
        for i, t in enumerate(timesteps, starting_step):
            ...
            interim_latents.append(latents)
            if len(interim_latents) == num_interim_images:
                    interim_images = torch.cat([self.vae_inference(lat * 1/0.18215) for lat in interim_latents])
                    interim_images = (interim_images / 2 + 0.5).clamp(0, 1)
                    interim_images = self.numpy_to_pil(interim_images.cpu().permute(0, 2, 3, 1).numpy())
                    return interim_images, latents, i+1
