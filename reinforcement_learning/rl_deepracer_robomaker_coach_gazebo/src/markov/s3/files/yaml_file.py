...
from markov.s3.utils import is_power_of_two
...
class YamlFile():
    ...
    def __init__(self, agent_type, bucket, s3_key,
        ...
        if not bucket or not s3_key:
            log_and_exit("yaml file S3 key or bucket not available for S3. \
        ...
        if self._agent_type == AgentType.ROLLOUT.value:
            ...
            log_and_exit("Unknown agent type in launch file: {}".format(self._agent_type),
                         SIMAPP_SIMULATION_WORKER_EXCEPTION,
                         SIMAPP_EVENT_ERROR_CODE_500)
    ...
    def get_yaml_values(self):
        ...
        if not self._yaml_values:
            ...
            if self._agent_type == AgentType.TOURNAMENT.value:
                self._validate_tournament_yaml_values()
            else:
    ...
    def _download(self):
        ...
        # download_params_and_roslaunch_agent.py and tournament_node.py
        self._s3_client.download_file(bucket=self._bucket,
                                      s3_key=self._s3_key,
                                      local_path=self._local_path)
        LOG.info("[s3] Successfully downloaded yaml file from s3 key {} to local {}.".
    ...
    def _postprocess_yaml_values(self):
        ...
        # set model metadata
        self._model_metadata_s3_keys = \
            self._yaml_values[YamlKey.MODEL_METADATA_FILE_S3_YAML_KEY.value]
        ...
        self._is_f1 = self._yaml_values.get(
            YamlKey.RACE_TYPE_YAML_KEY.value, RaceType.TIME_TRIAL.value) == F1_RACE_TYPE
    ...
    def _validate_yaml_values(self):
        ...
        if not all(map(lambda param: len(self._yaml_values[param]) == \
                len(self._yaml_values[self._mandatory_yaml_key[0]]), self._mandatory_yaml_key)):
            raise Exception('Incorrect number of values for these yaml parameters {}'.
                            format(self._mandatory_yaml_key))

        # Verify if all yaml keys have at least 2 values for multi car racing
        if self._is_multicar and \
                len(self._yaml_values[self._model_s3_prefix_yaml_key]) < 2:
            raise Exception('Incorrect number of values for multicar racing yaml parameters {}'.
                            format(self._mandatory_yaml_key))

        # Verify if all yaml keys have 1 value for single car racing
        if not self._is_multicar and \
                len(self._yaml_values[self._model_s3_prefix_yaml_key]) != 1:
            raise Exception('Incorrect number of values for single car racing yaml parameters {}'.
                            format(self._mandatory_yaml_key))

    def _validate_tournament_yaml_values(self):
        """ Validate that the parameter provided in the yaml file for configuration is correct.
        ...
        same_len_values = self._mandatory_yaml_key
        if None not in self._yaml_values.get(YamlKey.RACER_NAME_YAML_KEY.value, [None]):
            same_len_values.append(YamlKey.RACER_NAME_YAML_KEY.value)
        LOG.info(self._yaml_values)
        if not all(map(lambda param: len(self._yaml_values[param]) == \
                len(self._yaml_values[same_len_values[0]]), same_len_values)):
        ...
        if not is_power_of_two(len(self._yaml_values[self._model_s3_prefix_yaml_key])):
            raise Exception(
                'Incorrect number of values for tournament (Requires power of 2): Given {} '
                .format(len(self._yaml_values[self._model_s3_prefix_yaml_key])))
    ...
    def _update_car_color(self):
        ...
        if YamlKey.CAR_COLOR_YAML_KEY.value not in self._yaml_values:
            self._yaml_values[YamlKey.CAR_COLOR_YAML_KEY.value] = \
                [CarColorType.BLACK.value] * len(self._model_s3_buckets)
    ...
    def _update_body_shell(self):
        ...
        body_shell_types = force_list(self._yaml_values.get(YamlKey.BODY_SHELL_TYPE_YAML_KEY.value, [None]))
        racer_names = force_list(self._yaml_values.get(YamlKey.RACER_NAME_YAML_KEY.value, [None]))
        display_names = force_list(self._yaml_values.get(YamlKey.DISPLAY_NAME_YAML_KEY.value, [None]))
        ...
        if None in body_shell_types:
            ...
            if None in racer_names and None in display_names:
                body_shell_types = [BodyShellType.DEFAULT.value] *\
                    len(force_list(self._yaml_values[self._model_s3_bucket_yaml_key]))
            else:
                ...
                if None in racer_names:
                    body_shell_types = [BodyShellType.F1_2021.value if racer_alias
