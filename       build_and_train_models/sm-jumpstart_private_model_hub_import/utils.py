...
def package_llama_model(model_dir, output_path="model.tar.gz"):
    ...
    for file in os.listdir(model_dir):
        if file in required_files or file.endswith(".safetensors"):
            shutil.copy2(os.path.join(model_dir, file), os.path.join(temp_dir, file))
...
def upload_model_to_s3(
    bucket: str, prefix: str, local_model_path: str = "model.tar.gz"
) -> str:
    """
    ...
    Args:
        ...
        prefix (str): S3 prefix/folder path where the model will be uploaded
    ...
    try:
        ...
        s3_uri = session.upload_data(
            path=local_model_path, bucket=bucket, key_prefix=prefix
        )
...
def get_hub_content_document(
    ...
    try:
        ...
        response = sagemaker_client.describe_hub_content(
            HubName=hub_name, HubContentType="Model", HubContentName=hub_content_name
        )
