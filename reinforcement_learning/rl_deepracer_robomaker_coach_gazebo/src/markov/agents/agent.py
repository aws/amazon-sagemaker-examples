    ...
    def judge_action(self, action, agents_info_map):
        ...
        if hasattr(self._ctrl_, "reward_data_pub") and self._ctrl_.reward_data_pub is not None:
            ...
            if Input.CAMERA.value in raw_state and raw_state[Input.CAMERA.value] is not None:
                self._ctrl_.reward_data_pub.publish_frame(
                    raw_state[Input.CAMERA.value], action, reward
                )
            elif (
                Input.OBSERVATION.value in raw_state
                and raw_state[Input.OBSERVATION.value] is not None
            ):
                self._ctrl_.reward_data_pub.publish_frame(
                    raw_state[Input.OBSERVATION.value], action, reward
                )
