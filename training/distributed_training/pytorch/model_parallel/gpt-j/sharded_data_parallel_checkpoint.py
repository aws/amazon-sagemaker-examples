...
def get_optim_files(checkpoint_dir):
    optim_files = sorted(
        glob.glob(os.path.join(checkpoint_dir, "optimizer_*.pt")), key=natural_keys
    )
...
def parse_model_state(model_file, user_content_file, dtype):
    ...
    if "buffer_names" not in user_content:
        raise ValueError(
            f"{user_content_file} miss buffer_names to reconstruct the full state"
        )
    if "param_shapes" not in user_content:
        raise ValueError(
            f"{user_content_file} miss param_shapes to reconstruct the full state"
        )
...
def parse_optim_states(files, checkpoint_dir, dtype):
    ...
    for i, f in enumerate(files):
        ...
        states["fp32_flat_groups"] = [
            group.to(dtype) for group in states["fp32_flat_groups"]
        ]
...
def get_full_state_dict_from_sharded_data_parallel_checkpoint(
    checkpoint_dir, dtype=torch.float32, tag=None, remove_smp_prefix=True
):
    """
    ...
    sharded_data_parallel_size, flat_groups = parse_optim_states(
        optim_files, checkpoint_dir, dtype
    )
    ...
    for name, shape in param_shapes.items():
        ...
        partitioned_numel, partitioned_padding_numel = partitioned_param_info(
            unpartitioned_numel, sharded_data_parallel_size
        )
        ...
        state_dict[name] = (
            torch.cat(
                tuple(
                    flat_groups[i].narrow(0, offset, partitioned_numel)
    ...
    print(f"Reconstructed state dict with {total_params} params {total_numel} elements")
