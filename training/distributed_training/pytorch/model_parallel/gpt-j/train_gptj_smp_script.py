...
def parse_args():
    #1. Parse Arguments
    parser = HfArgumentParser((ModelArguments, DataTrainingArguments, TrainingArguments, SMPArguments))
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        ...
        model_args, data_args, training_args, smp_args = parser.parse_args_into_dataclasses()
...
def main():
    ...
    if is_sagemaker_mp_enabled():
        ...
        model = smp.DistributedModel(model, trace_device=smp_args.trace_device,  gradient_as_bucket_view=True)
        ...
        if training_args.use_adamw > 0:
            optimizer = training_args.AdamW(
                param_groups, betas=(training_args.beta1, training_args.beta2), lr=training_args.lr, weight_decay=training_args.weight_decay
            )
        else:
