...
class JoinJobRecord:
    ...
    def to_ddb_record(self):
        obs_start_time_str = (
            self._obs_start_time.strftime("%Y-%m-%d-%H")
            if self._obs_start_time is not None
            else None
        )
        obs_end_time_str = (
            self._obs_end_time.strftime("%Y-%m-%d-%H") if self._obs_end_time is not None else None
        )
        return {
    ...
    def load_from_ddb_record(cls, record):
        ...
        obs_end_time = (
            datetime.strptime(record["obs_end_time"], "%Y-%m-%d-%H")
            if record["obs_end_time"] is not None
            else None
        )
