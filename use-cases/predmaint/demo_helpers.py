...
def update_dw_s3uri(flow_file_name):
    ...
    s3_client.upload_file(Filename=f'data/{fleet_info_filename}', Bucket=bucket, Key=f'{prefix}/data/{fleet_info_filename}')
    s3_client.upload_file(Filename=f'data/{sensor_logs_filename}', Bucket=bucket, Key=f'{prefix}/data/{sensor_logs_filename}')
    ...
    for node in flow['nodes']:
        if node['type'] == 'SOURCE':
            if node['parameters']['dataset_definition']['name'] == fleet_info_filename:
                node['parameters']['dataset_definition']['s3ExecutionContext']['s3Uri'] = fleet_info_uri
            elif node['parameters']['dataset_definition']['name'] == sensor_logs_filename:
                node['parameters']['dataset_definition']['s3ExecutionContext']['s3Uri'] = sensor_logs_uri
