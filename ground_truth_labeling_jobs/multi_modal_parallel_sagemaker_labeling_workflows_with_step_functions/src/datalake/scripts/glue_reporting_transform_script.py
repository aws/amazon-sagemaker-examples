...
username_email_dict =  {x['Username']:y['Value'] for x in cog_users['Users'] for y in x['Attributes'] if y['Name']=='email'}
username_sub_id_dict = {y['Value']:x['Username'] for x in cog_users['Users'] for y in x['Attributes'] if y['Name']=='sub'}
...
if len(input_df.head(1)) > 0:
    ...
    new_df = input_df.withColumn("username", uname_udf(col("answers.workerMetadata.identityData.sub")))
    ...
    new_df = new_df.withColumn('identityProviderType',col("answers.workerMetadata.identityData.identityProviderType")[0])
    ...
    df3 = new_df.withColumn("year", year(col("submissionTime")))\
