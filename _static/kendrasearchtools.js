...
var KendraSearch = {
   ...
   * execute search (requires search index to be loaded)
   */
   query : function(query, pageNumber) {
    var url = "https://1q43ik7b39.execute-api.us-west-2.amazonaws.com/prod"
    // var url = "https://1q43ik7b39atatll.execute-api.us-west-2.amazonaws.com/prod"

    // array of [filename, title, anchor, descr, score]
    $('#search-progress').empty();

    console.log("query: ", query, "pageNumber:", pageNumber);

    fetch(url, {
      method: 'post',
      body: JSON.stringify({ "queryText": query , "pageNumber": pageNumber}),
    ...
    .then(function(data) {
      ...
      console.log('data["ResponseMetadata"]["HTTPStatusCode"]', data["ResponseMetadata"]["HTTPStatusCode"]);
      if(data["ResponseMetadata"]["HTTPStatusCode"] != 200){
        ...
        pageNumber += 1;
        var maxPaginationButtons = Math.min(8, Math.floor(data["TotalNumberOfResults"]/data["ResultItems"].length)+1);
        var startPaginationButtons = Math.max(1, pageNumber-(maxPaginationButtons/2));
        var endPaginationButtons = startPaginationButtons + maxPaginationButtons;
        var paginationItem = $('<div class="pagination"></div>');
        for(var i = startPaginationButtons; i < endPaginationButtons; i++){
          if(i == pageNumber){
        ...
        KendraSearch.output.append(paginationItem);

        $('.paginationnolink').each(function ( index, element ) {
          $(element).on('click', function() {
            console.log("CLICKED", $(element).attr('id').split("-")[1]);
            KendraSearch.output.empty();
            KendraSearch.query(query, parseInt($(element).attr('id').split("-")[1])-1);
          });
        });
      }     
