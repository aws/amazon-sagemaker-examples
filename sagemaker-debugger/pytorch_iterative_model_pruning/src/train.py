...
def loader(batch_size=128):
    ...
    # training data
    train_data = datasets.ImageFolder(os.environ["SM_CHANNEL_TRAIN"], transform=train_transforms)

    # validation data
    val_data = datasets.ImageFolder(os.environ["SM_CHANNEL_TEST"], transform=val_transforms)

    # train dataloader
    train_iterator = torch.utils.data.DataLoader(
        train_data, shuffle=True, num_workers=4, batch_size=batch_size
    )
    # validation dataloader
    valid_iterator = torch.utils.data.DataLoader(val_data, num_workers=4, batch_size=batch_size)
