...
class QwenVLAdapter(BaseVLMAdapter):
    ...
    def load_model(self, model_id: str, **kwargs):
        ...
        # Extract use_cache if present (Qwen3-VL doesn't accept it during init)
        ...
        # Set use_cache on config after loading if it was specified
    ...
    def process_image(
        ...
        Qwen-VL supports dynamic resolution and native aspect ratios.
        """
        # Qwen-VL uses structured message format for processing
        # We'll process just the image here
        inputs = processor(
            images=image,
            return_tensors="pt"
        )
    ...
    def get_lora_target_modules(self, include_vision: bool = False) -> List[str]:
        ...
        if include_vision:
            # Vision encoder targets (custom ViT)
            vision_targets = [
    ...
    def supports_dynamic_resolution(self) -> bool:
        """Qwen-VL supports dynamic resolution."""
        return True
    
    def get_default_image_size(self) -> int:
        """
        Qwen-VL can handle any resolution.
        
        Returns a common size, but model is flexible.
        """
        return 448
