...
class JoinDbClient(object):
    ...
    def get_join_job_record(self, experiment_id, join_job_id):
        response = self.table_session.query(
            ...
            & Key("join_job_id").eq(join_job_id),
        )
    ...
    def create_new_join_job_record(self, record):
        try:
            self.table_session.put_item(
                Item=record, ConditionExpression="attribute_not_exists(join_job_id)"
            )
    ...
    def update_join_job_input_obs_data_s3_path(
        self, experiment_id, join_job_id, input_obs_data_s3_path
    ):
        self.table_session.update_item(
            Key={"experiment_id": experiment_id, "join_job_id": join_job_id},
    ...
    def update_join_job_input_reward_data_s3_path(
        self, experiment_id, join_job_id, input_reward_data_s3_path
    ):
        self.table_session.update_item(
            Key={"experiment_id": experiment_id, "join_job_id": join_job_id},
