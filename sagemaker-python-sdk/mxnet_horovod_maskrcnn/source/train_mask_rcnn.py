...
def get_dataset( args):
    train_dataset = gdata.COCOInstance(root=args.datasetloc,splits='instances_train2017')
    val_dataset = gdata.COCOInstance(root=args.datasetloc,splits='instances_val2017', skip_empty=False)
    starting_id = 0
    ...
    val_metric = COCOInstanceMetric(val_dataset, args.save_prefix + '_eval',
                                        use_ext=False, starting_id=starting_id)
    if args.horovod and MPI:
...
def train(net, train_data, val_data, eval_metric, batch_size, ctx, logger, args):
    ...
    rpn_box_loss = mx.gluon.loss.HuberLoss(1./9.)  # == smoothl1
    rcnn_cls_loss = mx.gluon.loss.SoftmaxCrossEntropyLoss()
    rcnn_box_loss = mx.gluon.loss.HuberLoss(1.)  # == smoothl1
...
if __name__ == '__main__':
    ...
    net_name = '_'.join(('mask_rcnn', 'fpn', 'resnet50_v1b', 'coco'))
