...
MISSING_REQUIRED_ARGUMENTS_ERROR = "Missing required arguments (Required arguments: s3_bucket, s3_prefix, aws_region)."
...
class ResponseHandler(object):
    ...
    def simapp_error(e=None, session_info=None):
        logger.error("An error occured during SimApp validation. [session_info={}]".format(session_info), e)
    ...
    def argument_error(e=None, session_info=None):
        ...
        return flask.Response(response=json.dumps(MISSING_REQUIRED_ARGUMENTS_ERROR),
                              status=500, mimetype='application/json')

    @staticmethod
    def server_error(e=None, session_info=None):
        # 5XX errors
        logger.error("An error occurred during invoking sagemaker invocations API. [session_info={}]"
                     .format(session_info), e, exc_info=True)
        return flask.Response(response=json.dumps(UNKNOWN_ERROR), status=500, mimetype='application/json')
