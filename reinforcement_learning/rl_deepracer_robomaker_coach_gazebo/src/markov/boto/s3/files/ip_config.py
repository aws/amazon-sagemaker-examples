...
from markov.boto.s3.constants import (SAGEMAKER_WAIT_TIME,
                                      IP_ADDRESS_POSTFIX,
                                      IP_DONE_POSTFIX)
...
class IpConfig():
    ...
    def persist(self, s3_kms_extra_args):
        ...
        LOG.info("[s3] Successfully uploaded ip address to \
                 s3 bucket {} with s3 key {}.".format(self._bucket, self._s3_ip_address_key))
        # persist done second
        # if retry failed, s3_client upload_fileobj will log and exit 500
        self._s3_client.upload_fileobj(bucket=self._bucket,
                                       ...
                                       s3_kms_extra_args=s3_kms_extra_args)
        LOG.info("[s3] Successfully uploaded ip done to \
                 s3 bucket {} with s3 key {}.".format(self._bucket, self._s3_ip_done_key))
    ...
    def _download(self):
        ...
        try:
            ...
            LOG.info("[s3] Successfully downloaded ip config from \
                 s3 key {} to local {}.".format(self._s3_ip_address_key, self._local_path))
            with open(self._local_path) as file:
                self._ip_file = json.load(file)["IP"]
        except botocore.exceptions.ClientError as err:
            log_and_exit("Failed to download ip file: s3_bucket: {}, s3_key: {}, {}"\
                .format(self._bucket, self._s3_ip_address_key, err),
                         SIMAPP_SIMULATION_WORKER_EXCEPTION,
                         SIMAPP_EVENT_ERROR_CODE_500)
