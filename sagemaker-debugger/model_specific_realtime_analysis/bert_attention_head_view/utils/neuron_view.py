...
class NeuronView:
    ...
    def create(self):
        ...
        for token, x_i, y_i in zip(self.input_tokens, x, y):
            ...
            text2 = Label(x=x_i + 105, y=y_i, text=token, text_font_size="10pt")
        ...
        self.source_key = ColumnDataSource(
            data=dict(image=[key[: self.n_tokens, :]], x=[40], y=[0], dw=[25], dh=[self.n_tokens])
        )
        ...
        query_input[:, :] = np.nan
        query_input[self.query, :] = query[self.n_tokens - self.query - 1, :]
        self.source_query = ColumnDataSource(
            data=dict(image=[query_input], x=[10], y=[0], dw=[25], dh=[self.n_tokens])
        )
        img = Image(image="image", x="x", y="y", dw="dw", dh="dh", color_mapper=color_mapper)
        ...
        self.product = ColumnDataSource(
            data=dict(
                image=[product[: self.n_tokens, :]], x=[70], y=[0], dw=[25], dh=[self.n_tokens]
            )
        )
        img = Image(image="image", x="x", y="y", dw="dw", dh="dh", color_mapper=color_mapper)
        ...
        self.dot_product = ColumnDataSource(
            data=dict(
                image=[dot_product[: self.n_tokens, :]], x=[100], y=[0], dw=[2], dh=[self.n_tokens]
            )
        )
        img = Image(image="image", x="x", y="y", dw="dw", dh="dh", color_mapper=color_mapper)
