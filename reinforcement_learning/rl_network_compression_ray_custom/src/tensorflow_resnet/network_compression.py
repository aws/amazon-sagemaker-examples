...
class NetworkCompressionBase(object):
  ...
  must have been initialized first by the stub class that sub classes it."""
    ...
    graphs from memory """
        ...
        self.master_model = self.model_class.builder(inputs, None, ModeKeys.REFERENCE,
                                                     self.params)
        reference_params = self.params.copy()
        reference_params['name'] = 'reference'
        reference_params['teacher'] = self.params['name']
        self.current_model = self.model_class.builder(inputs, None, ModeKeys.REFERENCE,
                                                      reference_params)

    def _perform_actions(self, epochs=5,
        ...
        self._reset()
        estimator = tf.estimator_builder(name=self.params['name'],
                                         model_params=self.params,
                                         model=self.model_class,
                                         remove_layers=self.params['remove_layers'],
                                         ckpt=None)

        retval = tf.train(estimator=estimator,
    ...
    def _update_checkpoint(self):
        if len(self.checkpoints) < self.num_checkpoints:
            heapq.heappush(self.checkpoints, (self.reward, self.xfactor, self.accuracy, self.params['dir']))
        elif self.checkpoints[0][0] < self.reward:
            item = heapq.heapreplace(self.checkpoints, (self.reward, self.xfactor, self.accuracy, self.params['dir']))
            ...
            for item in self.checkpoints:
                self.metricsfile.write(str(item[0]) + "," + str(item[1]) + ", " + str(item[2]) + ", " + str(item[3]) + "\n")
            self.metricsfile.flush()
            for item in self.checkpoints:
                self.logfile.write(str(item[0]) + "," + str(item[1]) + ", " + str(item[2]) + ", " + str(item[3]) + "\n")
        else:
            return True
...
class NetworkCompression(NetworkCompressionBase):
  ...
  """
    ...
    ##### Public methods #####
    def __init__(self, prefix='', dataset=Cifar10,
                 model_class=ResNet18Model(_MASTER_MODEL_NAME,
                                           _PARAMS, scope=_PARAMS['params_scope']),
    ...
    """
        ...
        try:
            ...
            printstr = ('%d: CR: %f, X-factor: %f, Accuracy: %f, Reward: %f' %
                        (self.actions_taken, C, 1.0 / (1 - C), metrics['accuracy'], self.reward))
        except Exception as e:
            ...
            printstr = ('%d: CR: %f, %f, Accuracy: %f, Reward: %f' %
                        (self.actions_taken, C, 1.0 / (1 - C), -1, self.reward))
