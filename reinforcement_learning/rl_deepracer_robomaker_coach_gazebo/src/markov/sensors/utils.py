...
def get_observation_space(sensor):
    ...
    if (
        sensor == Input.CAMERA.value
        or sensor == Input.OBSERVATION.value
        or sensor == Input.LEFT_CAMERA.value
    ):
        obs[sensor] = ImageObservationSpace(
            shape=np.array((TRAINING_IMAGE_SIZE[1], TRAINING_IMAGE_SIZE[0], 3)),
            high=255,
            channels_axis=-1,
        )
    elif sensor == Input.STEREO.value:
        obs[sensor] = PlanarMapsObservationSpace(
            shape=np.array((TRAINING_IMAGE_SIZE[1], TRAINING_IMAGE_SIZE[0], 2)),
            low=0,
            high=255,
            channels_axis=-1,
        )
        ...
        obs[sensor] = VectorObservationSpace(
            shape=TRAINING_LIDAR_SIZE, low=0.15, high=SECTOR_LIDAR_CLIPPING_DIST
        )
    else:
    ...
    if network_type == NeuralNetwork.DEEP_CONVOLUTIONAL_NETWORK_SHALLOW.value:
        ...
        input_embedder = {
            Input.CAMERA.value: {
                ...
                SchemeInfo.BN_INFO_CONV.value: [False, ActivationFunctions.TANH.value, 0.0],
                SchemeInfo.BN_INFO_DENSE.value: [False, ActivationFunctions.TANH.value, 0.0],
                SchemeInfo.IS_FIRST_LAYER_BN.value: False,
            }
        }
        ...
        input_embedder = {
            Input.CAMERA.value: {
                SchemeInfo.CONV_INFO_LIST.value: [[32, 8, 4], [32, 4, 2], [64, 4, 2], [64, 3, 1]],
                SchemeInfo.DENSE_LAYER_INFO_LIST.value: [512, 512],
                SchemeInfo.BN_INFO_CONV.value: [True, ActivationFunctions.RELU.value, 0.0],
    ...
    if network_type == NeuralNetwork.DEEP_CONVOLUTIONAL_NETWORK_SHALLOW.value:
        ...
        input_embedder = {
            Input.LEFT_CAMERA.value: {
                ...
                SchemeInfo.DENSE_LAYER_INFO_LIST.value: [64],
                SchemeInfo.BN_INFO_CONV.value: [False, ActivationFunctions.TANH.value, 0.0],
                SchemeInfo.BN_INFO_DENSE.value: [False, ActivationFunctions.TANH.value, 0.3],
                SchemeInfo.IS_FIRST_LAYER_BN.value: False,
            }
        }
    elif network_type == NeuralNetwork.DEEP_CONVOLUTIONAL_NETWORK_DEEP.value:
...
def get_stereo_camera_embedders(network_type):
    ...
    #! TODO decide if we want to have a deep-deep topology that differes from deep
        ...
        network_type == NeuralNetwork.DEEP_CONVOLUTIONAL_NETWORK.value
