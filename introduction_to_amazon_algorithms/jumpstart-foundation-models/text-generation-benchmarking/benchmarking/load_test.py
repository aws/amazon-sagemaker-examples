...
def run_load_test(
    ...
    time.sleep(60.) # wait for 1 cloudwatch period to ensure no extra queries are reported
    time_utc_start = datetime.datetime.utcnow()
...
def run_benchmarking_load_tests(
    ...
    metrics = extract_cloudwatch_metrics(
        statistics_latency, model_id, payload_name, predictor.endpoint_name, retry_wait_time, max_total_retry_time,
    )
