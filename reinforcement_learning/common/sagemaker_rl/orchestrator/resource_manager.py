...
class ResourceManager(object):
    ...
    def __init__(self, resource_config, boto_session=None):
        ...
        self.shared_resource_stack_name = self._resource_config.get("shared_resource").get(
            "resources_cf_stack_name"
        )

        self.hosting_fleet_config = self._resource_config.get("private_resource").get(
            "hosting_fleet"
        )
        self.training_fleet_config = self._resource_config.get("private_resource").get(
            "training_fleet"
        )
        self.evaluation_fleet_config = self._resource_config.get("private_resource").get(
            "evaluation_fleet"
        )
    ...
    def _wait_for_cf_stack_create_to_complete(self):
        ...
        try:
            ...
            logger.error(e)
            logger.error(
                "Failed to Create Stack with name {} ".format(self.shared_resource_stack_name)
            )
            raise Exception(
    ...
    def _get_cf_output_by_key(self, output_key):
        ...
        """
        stack_json = self.cf_client.describe_stacks(StackName=self.shared_resource_stack_name)[
            "Stacks"
        ][0]
        ...
        raise Exception(
            "Cannot find an Output with OutputKey {} in Shared CF Stack".format(output_key)
        )
    ...
    def _wait_for_active_firehose(self, stream_name):
        ...
        while status != "ACTIVE" and timeout >= 0:
            ...
            try:
                result = self.firehose_client.describe_delivery_stream(
                ...
                raise RuntimeError(
                    f"Failed to describe delivery stream '{stream_name}' "
    ...
    def _init_firehose_from_config(
        self, stream_name, s3_bucket, s3_prefix, buffer_size=128, buffer_time=60
    ):
        ...
        """
        exist_delivery_streams = self.firehose_client.list_delivery_streams(Limit=1000)[
            "DeliveryStreamNames"
        ]
        if stream_name in exist_delivery_streams:
            logger.warning(
                f"Delivery stream {stream_name} already exist. " "No new delivery stream created."
            )
        else:
            ...
            try:
                ...
                message = e.response["Error"]["Message"]
                raise RuntimeError(
                    f"Failed to create delivery stream '{stream_name}' "
                    f"with error {error_code}: {message}"
                )
    ...
    def delete_firehose_stream(self, stream_name):
        ...
        try:
            ...
            message = e.response["Error"]["Message"]
            raise RuntimeError(
                f"Failed to delete delivery stream '{stream_name}' "
                f"with error {error_code}: {message}"
            )
