...
def _get_random_hyperparameter_values(hyperparameters):
    ...
    for hp, definition in hyperparameters.items():
        if isinstance(definition, (CategoricalParameter, IntegerParameter, ContinuousParameter)):
...
def random_search(train_fn, hyperparameters, base_name=None, max_jobs=100, max_parallel_jobs=100):
    """
    ...
    for i in range(max_jobs):
        ...
        while len(running_jobs) == max_parallel_jobs:
            for job in list(running_jobs):
                if (
                    client.describe_training_job(TrainingJobName=job)["TrainingJobStatus"]
                    != "InProgress"
                ):
...
def table_metrics(jobs, metrics):
    ...
    for job in list(job_metrics):
        ...
        job_metrics[job].update({"objective": objective, "job_number": int(job.split("-")[-1])})
