...
def parse_dataset_files(base_path,filename): 
    ...
    xml_files=[f'{base_path}/VOCdevkit/VOC2012/Annotations/{dataset_files[i]}.xml' for i in range(len(dataset_files))]
    result=[]
    for xml_file in xml_files:
        result.append(xml_data_parser(xml_file))
    return result
...
if __name__ == "__main__":
    ...
    train_dataset=parse_dataset_files(input_path,f'{input_path}/VOCdevkit/VOC2012/ImageSets/Main/train.txt') 
    val_dataset=parse_dataset_files(input_path,f'{input_path}/VOCdevkit/VOC2012/ImageSets/Main/val.txt') 
    print(f'{len(train_dataset)} samples for training and {len(val_dataset)} samples for validation before loading')
    ...
    print(f'{len(train_dataset)} samples for training and {len(val_dataset)} samples for validation after loading')
