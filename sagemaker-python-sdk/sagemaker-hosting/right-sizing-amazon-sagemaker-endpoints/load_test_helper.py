...
def run_load_tests(api_url, endpoints_list):
    ...
    for endpoint in endpoints_list:
        ...
        subprocess.run(
            ["bash", "run_locust.sh", f"{endpoint}", f"{api_url}"],
...
def get_pricing(instance_type, duration="hour"):
    ...
    for price_list in resp["PriceList"]:
        ...
        if (
            json.loads(price_list)["product"]["attributes"]["location"]
            == "US East (N. Virginia)"
        ):
...
def generate_plots(endpoints, endpoints_dict, results_folder, sep_cpu_gpu=False):
    ...
    for item in endpoints_dict:
        ...
        prices.update({f"{instance}.x{count}": cost * count})
    ...
    for ep in endpoints:
        ...
        max_requests.update({ep.split("-", 1)[1].replace("-", "."): fail_at_1})
    ...
    results["gpu_flag"] = results["type"].apply(
        lambda x: 1 if x.startswith(("p", "g", "e", "i")) else 0
    )
...
def get_min_max_instances(results_df, min_requests, max_requests):
    ...
    if max_requests < min_requests:
        print(
            "Minimum requests should be less than or equal to the maximum number of requests per second. Exiting.."
        )
        return
    ...
    results_df["Min Instances"] = results_df["Max Requests per Second"].apply(
        lambda x: round(min_requests / x)
    )
    results_df["Pricing"] = results_df.apply(
        lambda x: x["Price per Hour"] * x["Min Instances"], axis=1
    )
    results_df = results_df.sort_values(["Pricing"])
    results_df = results_df[results_df["Min Instances"] > 0]
    results_df["Max Instances"] = results_df["Max Requests per Second"].apply(
        lambda x: round(max_requests / x)
    )
