...
from markov.s3.constants import (CHECKPOINT_LOCAL_DIR_FORMAT,
...
class TensorflowModel():
    ...
    def _download(self, s3_key, local_path):
        ...
        LOG.info("[s3] Successfully downloaded {} from \
                 s3 key {} to local {}.".format(file_name,
                                                s3_key,
                                                local_path))
            ...
            for root, _, files in os.walk(self._local_dir):
                for filename in files:
                    ...
                    if filename.startswith(ckpt_state.name):
                        ...
                        check_point_key_list.append(os.path.normpath(os.path.join(self._s3_key_dir,
                                                                                  rel_name)))
    ...
    def rename(self, coach_checkpoint_state_file, agent_name):
        ...
        '''
        try:
            LOG.info("Renaming checkpoint from checkpoint_dir: {} for agent: {}"
    ...
    def persist_tensorflow_frozen_graph(self, agent_name, graph_manager, coach_checkpoint_state_file,
        ...
        frozen_graph_local_path = os.path.join(SM_MODEL_PB_TEMP_FOLDER,
                                               agent_name,
                                               frozen_name)
        ...
        LOG.info("saved intermediate frozen graph: %s",
    ...
    def delete(self, coach_checkpoint_state_file, best_checkpoint):
        ...
        '''
        if coach_checkpoint_state_file.read() and \
                self._delete_queue.qsize() > NUM_MODELS_TO_KEEP:
