...
class ModelManager:
    ...
    def _fit_first_model(
        self, input_data_s3_prefix=None, manifest_file_path=None, wait=False, logs=True
    ):
        """
        ...
        if manifest_file_path:
            ...
            self.rl_estimator.fit(
                job_name=self.model_id, inputs=input_data_s3_prefix, wait=wait, logs=logs
            )
    ...
    def evaluate(
        ...
        eval_data_s3_path = (
            manifest_file_path if (manifest_file_path is not None) else input_data_s3_prefix
        )
        ...
        if local_mode:
            ...
            with CaptureStdout() as log_output:
                self.rl_estimator.fit(
                    job_name=evaluation_job_name, inputs=eval_channel_inputs, wait=wait, logs=logs
                )
