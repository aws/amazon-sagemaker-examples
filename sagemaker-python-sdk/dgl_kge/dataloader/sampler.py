...
def create_neg_subgraph(pos_g, neg_g, is_pbg, neg_head, num_nodes):
    ...
    if (neg_head and len(neg_g.head_nid) == num_nodes) or (
        ...
        chunk_size = 1
    return PBGNegEdgeSubgraph(neg_g, num_chunks, chunk_size, neg_sample_size, neg_head)
    ...
    def __next__(self):
        while True:
            ...
            neg_g = create_neg_subgraph(
                pos_g, neg_g, "PBG" in self.mode, self.neg_head, self.g.number_of_nodes()
            )
...
class EvalDataset(object):
    def __init__(self, dataset, args):
        ...
        if args.pickle_graph and os.path.exists(
            ...
            if args.pickle_graph:
                with open(
                    os.path.join(args.data_path, args.dataset, pickle_name), "wb"
                ) as graph_file:
                    pickle.dump(g, graph_file)
        self.g = g
    ...
    def create_sampler(
        self, eval_type, batch_size, neg_sample_size, mode="head", num_workers=5, rank=0, ranks=1
    ):
        edges = self.get_edges(eval_type)
...
class NewBidirectionalOneShotIterator:
    def __init__(self, dataloader_head, dataloader_tail, is_pbg, num_nodes):
        ...
        self.iterator_head = self.one_shot_iterator(dataloader_head, is_pbg, True, num_nodes)
        self.iterator_tail = self.one_shot_iterator(dataloader_tail, is_pbg, False, num_nodes)
        self.step = 0
