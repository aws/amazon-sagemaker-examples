...
def get_feature_definitions(df, feature_group):
    ...
    for column in df.columns:
        feature_type = feature_group._DTYPE_TO_FEATURE_DEFINITION_CLS_MAP.get(str(df[column].dtype), None)
        feature_definitions.append(FeatureDefinition(column, feature_type)) # you can alternatively define your own schema
...
def ingest_df_to_feature_group(df, feature_group_name, feature_store_client):
    ...
    for row_num, row_series in df.astype(str).iterrows():
        ...
        response = feature_store_client.put_record(FeatureGroupName=feature_group_name, Record=record)
...
def create_feature_group(feature_group_name, feature_group_description, df,
                         id_name, event_time_name, offline_feature_group_bucket,
