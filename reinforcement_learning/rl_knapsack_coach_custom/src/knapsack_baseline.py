        ...
        for j in range(0, c_max+1):
            if i == T:
                ...
                if j < weights[i-1]:
                    ...
                    reward[i-1][j] = max(-penalty + reward[i][j], values[i-1] + reward[i][j-weights[i-1]])
...
def find_sol_simple(i, c_w, reward, weights, values, penalty):
    ...
    if i == T:
        ...
        if reward[i-1][c_w] == -penalty + reward[i][c_w]:
            ...
            return [1] + find_sol_simple(i+1, c_w - weights[i-1], reward, weights, values, penalty)


def test_sol_valid_simple(knapsack_value, optimal_packing, weights, values, c_max, penalty):
    if (np.dot(optimal_packing, values) - penalty*(len(optimal_packing) -np.sum(optimal_packing))) != knapsack_value:
...
def get_knapsack_solution_medium(weights, volumes, values, c_weight_max, c_vol_max, penalty, debug=False):
    if (len(weights) != len(values)) or (len(volumes) != len(weights)) :
        print('values, volumes, weights vectors do not have the same length')
        return

    T = len(weights)
    reward = [[[0 for z in range(0, c_vol_max+1)] for x in range(0, c_weight_max+1)] for y in range(1,T+1)]
    #note: reward[i][c_w] has the reward in period (i+1) with residual capacity c_w
            ...
            for z in range(0, c_vol_max+1):
                if i == T:
                    ...
                    if j < weights[i-1] or z < volumes[i-1]:
                        ...
                        reward[i-1][j][z] = max(-penalty + reward[i][j][z], values[i-1] + reward[i][j-weights[i-1]][z - volumes[i-1]])

    knapsack_value = reward[0][c_weight_max][c_vol_max]
    optimal_packing = find_sol_medium(1, c_weight_max, c_vol_max, reward, volumes, weights, values, penalty)

    if debug:
        test_sol_valid_medium(knapsack_value, optimal_packing, weights, volumes, values, c_weight_max, c_vol_max, penalty)
...
def find_sol_medium(i, c_w, c_v, reward, volumes, weights, values, penalty):
    ...
    if i == T:
        ...
        if reward[i-1][c_w][c_v] == -penalty + reward[i][c_w][c_v]:
            ...
            return [1] + find_sol_medium(i+1, c_w - weights[i-1], c_v - volumes[i-1], reward, volumes, weights, values, penalty)

def test_sol_valid_medium(knapsack_value, optimal_packing, weights, volumes, values, c_weight_max, c_vol_max, penalty):
    if (np.dot(optimal_packing, values) - penalty*(len(optimal_packing) - np.sum(optimal_packing))) != knapsack_value:
        print('total solution value does not match up to knapsack value.')
...
def get_knapsack_solution_hard(weights, volumes, values, c_weight_max, c_vol_max, penalty, duration, debug=False):
    ...
    T = len(weights)
    reward = [[[{} for z in range(0, c_vol_max+1)] for x in range(0, c_weight_max+1)] for y in range(1,T+1)]
    #note: reward[i][c_w] has the reward in period (i+1) with residual capacity c_w
                ...
                for w in range(0, 2 ** (min(duration, i-1))):
                    ...
                    if i == T:
                        ...
                        if i > duration:
                            ...
                            if j_hat < weights[i-1] or z_hat < volumes[i-1]:
                                past_action_to_reward_map[key] = -penalty + reward[i][j_hat][z_hat][new_key1]
                            else:
                                past_action_to_reward_map[key] = max(-penalty + reward[i][j_hat][z_hat][new_key1], values[i-1] + reward[i][j_hat-weights[i-1]][z_hat - volumes[i-1]][new_key2])
                        elif i > 1:
                            if j < weights[i-1] or z < volumes[i-1]:
                                past_action_to_reward_map[key] = -penalty + reward[i][j][z][key + str(0)]
                            else:
                                past_action_to_reward_map[key] = max(-penalty + reward[i][j][z][key + str(0)], values[i-1] + reward[i][j-weights[i-1]][z - volumes[i-1]][key + str(1)])
                        else:
                            if j < weights[i-1] or z < volumes[i-1]:
                                past_action_to_reward_map[key] = -penalty + reward[i][j][z][str(0)]
                            else:
                                past_action_to_reward_map[key] = max(-penalty + reward[i][j][z][str(0)], values[i-1] + reward[i][j-weights[i-1]][z- volumes[i-1]][str(1)])



    knapsack_value = reward[0][c_weight_max][c_vol_max]['0']
    optimal_packing = find_sol_hard(i, c_weight_max, c_vol_max, '0', reward, weights, volumes, values, penalty, duration)

    if debug:
        test_sol_valid_hard(knapsack_value, optimal_packing, weights, volumes, values, c_weight_max, c_vol_max, penalty, duration)
    ...
    if i == T:
        ...
        if i > duration:
            ...
            if past_action_to_reward_map[key] == -penalty + reward[i][j_hat][z_hat][new_key1]:
                return [0] + find_sol_hard(i+1, j_hat, z_hat, new_key1, reward, weights, volumes, values, penalty, duration)
            else:
                return [1] + find_sol_hard(i+1, j_hat-weights[i-1], z_hat - volumes[i-1], new_key2, reward, weights, volumes, values, penalty, duration)
        elif i > 1:
            if past_action_to_reward_map[key] == -penalty + reward[i][c_w][c_v][key + str(0)]:
                return [0] + find_sol_hard(i+1, c_w, c_v, key + str(0), reward, weights, volumes, values, penalty, duration)
            else:
                return [1] + find_sol_hard(i+1, c_w - weights[i-1], c_v - volumes[i-1], key + str(1), reward, weights, volumes, values, penalty, duration)
        else:
            if past_action_to_reward_map[key] == -penalty + reward[i][c_w][c_v][str(0)]:
                return [0] + find_sol_hard(i+1, c_w, c_v, str(0), reward, weights, volumes, values, penalty, duration)
            else:
                return [1] + find_sol_hard(i+1, c_w - weights[i-1], c_v - volumes[i-1], str(1), reward, weights, volumes, values, penalty, duration)


def test_sol_valid_hard(knapsack_value, optimal_packing, weights, volumes, values, c_weight_max, c_vol_max, penalty, duration):
    if (np.dot(optimal_packing, values) - penalty*(len(optimal_packing)-np.sum(optimal_packing))) != knapsack_value:
        print('total solution value does not match up to knapsack value.')
        print((np.dot(optimal_packing, values) - penalty*(len(optimal_packing)-np.sum(optimal_packing))))
...
def get_knapsack_benchmark_sol_hard_greedy_heuristic(weights, volumes, values, c_weight_max, c_vol_max, penalty, duration, debug=False):
    if (len(weights) != len(values)) or (len(volumes) != len(weights)) :
        print('values, volumes, weights vectors do not have the same length')
        return
    ...
    for j in range(1, max_trials+1):
        ...
        for i in range(0,len(weights)):
            ...
            if (values[i]/weights[i] < threshold) or (values[i]/volumes[i] < threshold):
                ...
                if (total_weight_in_knapsack + weights[i] <= c_weight_max) and (total_volume_in_knapsack + volumes[i] <= c_vol_max):
...
if __name__ == "__main__":
    ...
    print(get_knapsack_benchmark_sol_hard_greedy_heuristic(weights, volumes, values, c_weight_max, c_vol_max, penalty, duration))
