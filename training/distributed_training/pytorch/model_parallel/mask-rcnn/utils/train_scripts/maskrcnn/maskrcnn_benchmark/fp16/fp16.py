...
class FP16_Optimizer(object):
    ...
    **Multi_GPU training**:  If the wrapped ``init_optimizer`` was created from a model wrapped in
    ...
    def init_master_params(self):
            ...
            for i, param in enumerate(param_group['params']):
                if param.requires_grad:
                    if param.type() in ['torch.cuda.HalfTensor', 'torch.HalfTensor']:
                        ...
                        master_param = param.detach().clone().float().to(torch.device("cuda"))
        ...
        if self.dynamic_loss_scale:
            if self.dynamic_loss_args is not None:
                ...
                self.loss_scaler = DynamicLossScaler(self.model, **self.dynamic_loss_args)
            else:
                self.loss_scaler = DynamicLossScaler(self.model, use_smp=self.use_smp)
        else:
            self.loss_scaler = LossScaler(self.model, self.static_loss_scale, use_smp=self.use_smp)
    ...
    def clip_master_grads(self, max_norm, norm_type=2, use_smp=False):
        """
        ...
        if not self.overflow:
            ...
            return self.clip_grad_norm(fp32_params, max_norm, norm_type, use_smp)
        else:
            return -1
    ...
    def backward(self, loss, update_master_grads=False, retain_graph=False):
        """
