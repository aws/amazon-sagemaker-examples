...
def main():
    ...
    train = reduce(
        lambda train, idx: train.withColumnRenamed(oldColumns[idx], newColumns[idx]),
    ...
    assembler = VectorAssembler(
        inputCols=["buyingVec", "maintVec", "doorsVec", "personsVec", "lug_bootVec", "safetyVec"],
        outputCol="features",
    )
    ...
    lines.saveAsTextFile(
        "s3a://" + args["s3_output_bucket"] + "/" + args["s3_output_bucket_prefix"] + "/" + "train"
    )
    ...
    SimpleSparkSerializer().serializeToBundle(
        converter, "jar:file:/tmp/postprocess.zip", predictions
    )
